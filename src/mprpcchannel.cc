...
*/
//所有通过stub代理对象调用的rpc方法都走到这里了，统一做rpc方法调用的数据参数序列化和网络发送
...
{  
    ...
    std::string args_str;
    if (request->SerializeToString(&args_str)) {//表示序列化成功
    ...
    //组织待发送的rpc请求的字符串
    std::string send_rpc_str;//在做请求参数的序列化
    ...
    if (-1 == clientfd) {
        ...
        char errtxt[512] = {0};
        sprintf(errtxt, "create socket erroe! errno: %d", errno);
    ...
    if (-1 == connect(clientfd, (struct sockaddr*)&server_addr, sizeof(server_addr)))
    {
        ...
        close(clientfd);
        char errtxt[512] = {0};
        sprintf(errtxt, "connect erroe! errno: %d", errno);
        controller->SetFailed(errtxt);
        return;
    };
    //发送rpc请求
    if (-1 == send(clientfd, send_rpc_str.c_str(), send_rpc_str.size(), 0)) {
        ...
        close(clientfd);
        char errtxt[512] = {0};
        sprintf(errtxt, "send erroe! errno: %d", errno);
        controller->SetFailed(errtxt);
        return;//结束本次rpc调用
    }
    //接收rpc请求的响应值
